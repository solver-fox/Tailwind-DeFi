...
export default function TokenTable() {
  ...
  const trendDisplay = (
    ...
    return (
      <span
        className={classNames(d1 > d90 ? "text-green-300" : "text-red-300")}
      >
        {latest?.length > 0 ? prepend : ""}
        {formatEth(latest)}
      </span>
    );
  };
  ...
  const onSearch = (input: string) => {
    if (input.length > 0) {
      ...
      const result = filteredTokens.filter(
        (x) => x.name.toLowerCase().indexOf(searchFor) > -1
      );
    ...
    <div className="px-4">
      <div className="mt-8 flex flex-col">
        ...
        <div className="mb-4 flex w-full">
          ...
          <div className="flex-1 text-right pt-6  text-gray-400 text-sm">
            Last refreshed 2/5/2023
          </div>
        </div>
                    ...
                    <tr key={index}>
                      <td className="py-4 whitespace-nowrap pl-4 pr-3 text-white sm:pl-6">
                        <div className="flex items-center gap-x-4">
                          ...
                          />
                          <div className="truncate leading-6 text-white">
                            {token.name}
                          </div>
                        </div>
                      </td>
                      <td className="whitespace-nowrap px-2 py-2 text-white text-right">
                        {formatEth(
                          token.metric_aggregations.price.values.latest
                        )}
                      </td>
                      <td className="whitespace-nowrap px-2 py-2 text-right">
                        {trendDisplay(token?.metric_aggregations.price.changes)}
                      </td>
                      <td className="whitespace-nowrap px-2 py-2 text-white text-right">
                        {formatLargeUsd(
                          token.metric_aggregations.tvl.values.latest
                        )}
                      </td>
                      <td className="whitespace-nowrap px-2 py-2 pr-8 text-white text-right">
                        {formatLargeUsd(
                          token.metric_aggregations.token_trading_volume.values
                            .latest
                        )}
                      </td>
                    </tr>
